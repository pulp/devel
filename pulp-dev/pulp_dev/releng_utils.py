...
def get_spec_field(specfile, rpmfield):
    # Get the dep name & version
    spec = rpm.spec(specfile)
    value = spec.sourceHeader[rpmfield]
    # this assumes that the rpm fields are decodable as utf8,
    # which they should should be... (*crosses fingers*)
    return value.decode('utf-8')


get_spec_version = functools.partial(get_spec_field, rpmfield=rpm.RPMTAG_VERSION)
...
def get_spec_release(specfile):
    ...
    :rtype: str
    """
    release = get_spec_field(specfile, rpm.RPMTAG_RELEASE)
    # strip the dist (e.g. '.fc23') from the end of the release string
    return release.rsplit('.', 1)[0]
